...
function App() {
  ...
  useEffect(() => {
    ...
    const savedUser =
      localStorage.getItem("agentsamosa_user") ||
      sessionStorage.getItem("agentsamosa_user");
    if (savedUser) {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/command/suggestions?query=${encodeURIComponent(command)}&role=${
            user.role
          }&limit=5`
        );
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (rememberMe) {
          localStorage.setItem("agentsamosa_user", JSON.stringify(data.user));
        } else {
          sessionStorage.setItem("agentsamosa_user", JSON.stringify(data.user));
        }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setUser(data);
        localStorage.setItem("agentsamosa_user", JSON.stringify(data));
  ...
  const logout = () => {
    setUser(null);
    localStorage.removeItem("agentsamosa_user");
    sessionStorage.removeItem("agentsamosa_user");
  ...
  const fetchShopsByCategory = async (categoryId, page = 0, reset = false) => {
    ...
    setIsLoading(true);
    try {
      let url = `/api/shops/by-category/${categoryId}?skip=${
        page * PAGE_SIZE
      }&limit=${PAGE_SIZE}`;
  ...
  const fetchShopProducts = async (shopId, page = 0, reset = false) => {
    ...
    setIsLoading(true);
    try {
      let url = `/api/shops/${shopId}/products?skip=${
        page * PAGE_SIZE
      }&limit=${PAGE_SIZE}`;
  ...
  const fetchAdminProducts = async (shopId, page = 0, reset = false) => {
    ...
    setIsLoading(true);
    try {
      let url = `/api/shops/${shopId}/products?include_inactive=true&skip=${
        page * PAGE_SIZE
      }&limit=${PAGE_SIZE}`;
      if (adminProductSearch)
        url += `&search=${encodeURIComponent(adminProductSearch)}`;
      const res = await fetch(url);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchAdminOrders = async (shopId, page = 0, reset = false) => {
    ...
    setIsLoading(true);
    try {
      let url = `/api/shops/${shopId}/orders?skip=${
        page * PAGE_SIZE
      }&limit=${PAGE_SIZE}`;
  ...
  const fetchAllShops = async (page = 0, reset = false) => {
    ...
    setIsLoading(true);
    try {
      let url = `/api/platform/shops?skip=${
        page * PAGE_SIZE
      }&limit=${PAGE_SIZE}`;
  ...
  const addLog = (message, type = "info") => {
    setLogs((prev) =>
      [{ message, type, time: new Date().toLocaleTimeString() }, ...prev].slice(
        0,
        30
      )
    );
    ...
    try {
      const data = {
        ...
        cost_price: productForm.cost_price
          ...
          : null,
        min_price: productForm.min_price
          ? parseFloat(productForm.min_price)
          : null,
  ...
  const verifyShop = async (shopId) => {
    const res = await fetch(`/api/platform/shops/${shopId}/verify`, {
      method: "PATCH",
    });
    if (res.ok) {
  ...
  const suspendShop = async (shopId) => {
    ...
    const res = await fetch(`/api/platform/shops/${shopId}/suspend`, {
      method: "PATCH",
    });
    if (res.ok) {
  ...
  const activateShop = async (shopId) => {
    const res = await fetch(`/api/platform/shops/${shopId}/activate`, {
      method: "PATCH",
    });
    if (res.ok) {
            ...
            showResetPassword ? (
                  ...
                  {resetForm.password && (
                    <div className="password-strength">
                        ...
                        {[1, 2, 3, 4, 5].map((level) => (
                          <div
                            ...
                            className={`strength-bar ${
                              level <=
                              getPasswordStrength(resetForm.password).score
                                ? "active"
                                : ""
                            }`}
                            style={{
                              backgroundColor:
                                level <=
                                getPasswordStrength(resetForm.password).score
                                  ? getPasswordStrength(resetForm.password)
                                      .color
                                  : "",
                            }}
                          />
                      ...
                      <span
                        ...
                        style={{
                          color: getPasswordStrength(resetForm.password).color,
                        }}
                      >
              ...
              <form onSubmit={handleForgotPassword} className="login-form">
                ...
                {forgotMessage && (
                  <div
                    className={
                      forgotMessage.includes("generated")
                        ? "success-msg"
                        : "info-msg"
                    }
                  >
                ...
                {registerForm.password && (
                  <div className="password-strength">
                    <div className="strength-bars">
                      {[1, 2, 3, 4, 5].map((level) => (
                        <div
                          ...
                          className={`strength-bar ${
                            level <=
                            getPasswordStrength(registerForm.password).score
                              ? "active"
                              : ""
                          }`}
                          style={{
                    ...
                    <span
                      className="strength-label"
                      style={{
                        color: getPasswordStrength(registerForm.password).color,
                      }}
                    >
            ...
            <div className="demo-accounts">
              ...
              <div className="demo-list">
                <button
                  onClick={() =>
                    setLoginForm({
                      email: "superadmin@agentsamosa.com",
                      password: "qwert12345",
                    })
                  }
                >
                ...
                <button
                  onClick={() =>
                    setLoginForm({
                      email: "customer@agentsamosa.com",
                      password: "qwert12345",
                    })
                  }
                >
        ...
        <div className="stats-grid platform-stats">
          ...
          <div className="stat-card success">
            <div className="stat-value">
              {platformStats.verified_shops || 0}
            </div>
            <div className="stat-label">Verified Shops</div>
          </div>
          <div className="stat-card">
            <div className="stat-value">
              {platformStats.total_shop_owners || 0}
            </div>
            <div className="stat-label">Shop Owners</div>
          </div>
          <div className="stat-card">
            <div className="stat-value">
              {platformStats.total_customers || 0}
            </div>
            <div className="stat-label">Customers</div>
          </div>
          <div className="stat-card success">
            <div className="stat-value">
              ${platformStats.platform_revenue?.toLocaleString() || 0}
            </div>
          ...
          <div className="tab-content">
                ...
                <form onSubmit={submitShopForm}>
                  ...
                  <div className="form-actions">
                    <button type="submit" className="submit-btn">
                      {editingShop ? "Update Shop" : "Register Shop"}
                    </button>
                    <button
                      type="button"
            ...
            <div className="data-panel">
              <SearchFilterBar
                ...
                placeholder="Search shops by name, email, city..."
              />
                ...
                <table>
                  ...
                  <tbody>
                    {allShops.map((shop) => (
                      <tr
                        key={shop.id}
                        className={!shop.is_active ? "suspended" : ""}
                      >
                        <td>
              ...
              <LoadMoreButton
                hasMore={allShopsHasMore}
                isLoading={isLoading}
                onClick={() => fetchAllShops(allShopsPage + 1)}
              />
            </div>
          </div>
        )}
                ...
                <div className="alert-list">
                  {lowStockProducts.map((p) => (
                    <div key={p.id} className="alert-item">
                      ...
                      <button
                        onClick={() =>
                          editProduct(products.find((prod) => prod.id === p.id))
                        }
                      >
                        Restock
                      </button>
          ...
          <div className="tab-content">
              ...
              <form onSubmit={editingProduct ? updateProduct : createProduct}>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    placeholder="e.g. lipstick, makeup"
                  />
                </div>
                <div className="form-actions">
                  {editingProduct && (
            ...
            <div className="data-panel">
              <SearchFilterBar
                ...
                setSearch={setAdminProductSearch}
                    ...
                    {products.map((p) => {
                      const margin =
                        p.cost_price && p.price
                          ? Math.round(
                              ((p.price - p.cost_price) / p.cost_price) * 100
                            )
                          : null;
                      return (
                        <tr
                          key={p.id}
                          className={!p.is_active ? "inactive" : ""}
                        >
                          <td>
              ...
              <LoadMoreButton
                hasMore={adminProductsHasMore}
                isLoading={isLoading}
                onClick={() =>
                  fetchAdminProducts(user.shop_id, adminProductsPage + 1)
                }
              />
            </div>
          </div>
        ...
        {activeTab === "orders" && (
          <div className="orders-panel">
                  ...
                  <tbody>
                    {orders.map((o) => (
                      <tr key={o.id}>
                        ...
                        <td>{o.quantity}</td>
                        <td className="price">
                          ₹{o.listed_price || o.unit_price}
                        </td>
                        <td className="price">
                          ₹{o.final_price || o.unit_price}
                        </td>
                        <td
                          className={`profit ${
                            (o.profit || 0) >= 0 ? "positive" : "negative"
                          }`}
                        >
            ...
            <LoadMoreButton
              ...
              isLoading={isLoading}
              onClick={() => fetchAdminOrders(user.shop_id, ordersPage + 1)}
            />
          </div>
        )}
      </div>
      ...
      <div className="marketplace">
        ...
        <div className="search-bar">
          <input
            ...
            value={shopsSearch}
            onChange={(e) => setShopsSearch(e.target.value)}
          />
        </div>
        <div className="shops-grid">
          {shops
            .filter(
              (s) =>
                !shopsSearch ||
                s.name.toLowerCase().includes(shopsSearch.toLowerCase())
            )
            .map((shop) => (
              <div
                key={shop.id}
          ...
          {shops.length === 0 && (
            <p className="empty">No shops in this category yet</p>
          )}
        </div>
        <LoadMoreButton
          hasMore={shopsHasMore}
          isLoading={isLoading}
          onClick={() =>
            fetchShopsByCategory(selectedShopCategory, shopsPage + 1)
          }
        />
      </div>
    );
    ...
    return (
      <div className="marketplace">
        ...
        <div className="products-grid">
          {shopProducts.map((p, index) => (
            <div
              ...
              ref={index === shopProducts.length - 1 ? lastItemRef : null}
            >
        ...
        <LoadMoreButton
          ...
          isLoading={isLoading}
          onClick={() => fetchShopProducts(selectedShop, productsPage + 1)}
        />
        {cart.length > 0 && (
